...
# Apply a negative badge to a fin based on failure roll tier
# - PURPLE: no badge
# - GREEN: base badge (-1)
# - GREY: severe badge (-2)
# - RED: will be crash (handled separately)
static func apply_badge_based_on_tier_to_fin(fin_state, base_badge_id: String, tier: Dice.Tier) -> bool:
	if base_badge_id == "":
		return false
	...
	return apply_badge_to_fin(fin_state, badge_id_to_apply)

# Apply a negative badge to a fin (adds to temporary_badges)
static func apply_badge_to_fin(fin_state, badge_id: String) -> bool:
	var badge = load_badge(badge_id)
	if badge == null:
		return false

	# Check if fin already has this badge
	for existing_badge in fin_state.temporary_badges:
		if existing_badge.badge_id == badge_id:
			# Already has this badge, don't add duplicate
			return false
